...
map<string, vector<double> > paramValues;
...
void *run (void* arg){
	...
	vector<double> values = td->values;
...
int main(){
	...
	float t_sv = 0.2; // similarity threshold for similar vector estimation: similarity if difference is below threshold
	short r_sn = 1; // Neighbour radius for similar neighbour weighting
	double t_sn = 0.75; // percentage threshold for similar neighbour weights
	short r_mr = 2; // radius of structuring element for dilation during morphological reconstruction

	vector<double> values;

	// values.clear();
	// for(double v=1; v<=5; v+=1) values.push_back(v);
	// paramValues[P_MIN_VEC] = vector<double>(values);
	...
	paramValues[P_T_SN] = vector<double>(values);

	// values.clear();
	// for(double v=1; v<=5; v+=1) values.push_back(v);
	// paramValues[P_R_MR] = vector<double>(values);
